document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!container) return;
  // For static deployments (e.g. GitHub Pages), load the JSON directly from the data folder.
  fetch('data/schedule.json')
    ...
    .then((days) => {
      days.forEach((day) => {
        ...
        // Wrap the table in a responsive container so it scrolls horizontally on small screens
        const responsiveWrapper = document.createElement('div');
        responsiveWrapper.className = 'table-responsive';
        responsiveWrapper.appendChild(table);
        dayDiv.appendChild(responsiveWrapper);
